...
class SwinBottle(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        # self.transformer = nn.Sequential(*[SwinTransformerBlock(c1, c1, c1 // 16, n) for _ in range(n)])
        self.transformer = SwinTransformerBlock(c1, 2 * self.c, 2 * self.c // 16, n)
    ...
    def forward(self, x):
        ...
        y = list(self.transformer(x).chunk(2, 1))
...
class CS_Attention(nn.Module):
    def __init__(self, in_channels, rate=2):
        ...
        self.channel_attention = CoordAtt(in_channels, in_channels)
    ...
    def forward(self, x):
        ...
        x_channel_att = self.batchnorm(x_channel_att)
        x = x * x_channel_att
...
class FocusSamp(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(FocusSamp, self).__init__()
...
class UpFocusSamp(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(UpFocusSamp, self).__init__()
